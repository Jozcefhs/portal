...
test('web and desktop expose the same controlled imprest lifecycle', () => {
  ...
  assert.match(adminJs, /const imprestReportDialog =/);
  assert.match(adminJs, /id="imprestReportDialog"/);
  ...
  assert.doesNotMatch(adminJs.slice(reportPreviewStart, reportPrintStart), /window\.open/);
  ...
  assert.match(portalCss, /\.imprest-report-dialog/);
