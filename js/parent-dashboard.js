...
function academicResultPrintMarkup(child, record, options = {}) {
  ...
  const primary = String(record.SchoolStage || child.SchoolSection || '').toLowerCase() === 'primary'
    || /(?:nursery|primary|grade\s*[1-6]\b)/i.test(String(record.ClassName || child.ClassName || ''));
  const learnerLabel = primary ? 'Pupil' : 'Student';
  const executiveLabel = primary ? 'Head Teacher' : 'Principal';
  const reportTitle = clean(record.ResultType) === 'Mid-Term' ? 'Mid-Term Academic Result' : 'Academic Result';
...
function renderAcademicResults(child) {
  ...
  records.forEach((record) => {
    ...
    item.innerHTML = `<header><div><strong>${escapeHtml([record.ResultType || 'End of Term', record.Term, record.AcademicSession].filter(Boolean).join(' · ') || 'Academic result')}</strong><span>${escapeHtml(record.ClassName || '')}</span></div><span>${record.Access?.Allowed ? 'Available' : 'Restricted'}</span></header>`;
