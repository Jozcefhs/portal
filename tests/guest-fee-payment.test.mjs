...
test('guest authorization is accepted only by payment APIs and never by the parent dashboard', async () => {
  ...
  assert.match(landingCss, /\.landing-hero \.hero-service-panel \.summary-icon\s*\{[\s\S]*?display: inline-flex;[\s\S]*?align-items: center;[\s\S]*?justify-content: center;[\s\S]*?align-self: center;/);
  assert.match(landingCss, /\.landing-hero \.hero-service-panel \.summary-icon\s*\{[\s\S]*?box-sizing: border-box;[\s\S]*?margin: 0;[\s\S]*?line-height: 1;[\s\S]*?text-align: center;/);
