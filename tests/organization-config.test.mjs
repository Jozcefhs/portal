...
test('church is canonicalized to faith while retaining shared finance modules', () => {
  ...
  assert.deepEqual(
    ...
    ['recordsDesk', 'executiveOffice', 'incomeAnalytics', 'members', 'services', 'funds', 'offerings', 'donations', 'financeRequests', 'payroll', 'organizationStore', 'restaurant', 'hotel', 'dataBackup', 'securityAudit', 'staffUsers', 'humanResources', 'staffAttendance']
  );
});
...
test('public organisation registration stays aligned and legible in both themes', async () => {
  ...
  assert.match(html, /20260809-platform-direct-transfer/);
