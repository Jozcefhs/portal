...
function renderFlexBuilder() {
  ...
  flexPlanBuilder.innerHTML = `
    ...
    <div class="flex-module-options">${flexModules().map((module) => {
      ...
      return `<label class="flex-module-option ${selected.has(module.Key) ? 'selected' : ''}"><input type="checkbox" data-flex-module="${escapeHtml(module.Key)}" ${selected.has(module.Key) ? 'checked' : ''} ${required ? 'data-required-by-selection="true"' : ''}><span class="flex-module-copy"><span class="flex-module-title"><strong>${escapeHtml(module.Label)}</strong><b>${escapeHtml(dualFormattedPrice(price, currency))}</b></span><small>${escapeHtml(module.Description)}</small>${module.Requires?.length ? `<em>Requires ${escapeHtml(module.Requires.map((key) => flexModules().find((entry) => entry.Key === key)?.Label || key).join(', '))}</em>` : ''}</span></label>`;
    }).join('')}</div>
